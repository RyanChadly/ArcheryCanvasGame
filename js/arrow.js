...
Arrow.prototype.fireArrow = function () {
  if (mousePos && !this.firing) {
    this.speed =
      Math.min(shootingCirc.r, distBetween(shootingCirc, mousePos)) / speedMod;
...
Arrow.prototype.calcArrowHead = function () {
  ...
  let arrowTip = { x: this.arrowTipCoords.x, y: this.arrowTipCoords.y };
...
Arrow.prototype.drawArrow = function () {
  this.calcTrajectory();
  this.calcArrowHead();
  let arrowTip = this.arrowTipCoords;
  let leftTip = this.leftTipCoords;
  let rightTip = this.rightTipCoords;
